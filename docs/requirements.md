...
- **Tests**: pure functions (slug, args, prompt rendering, branch naming) covered directly with `bun:test`. I/O modules use the fixtures under `tests/helpers/` — `scriptedSpawn` for `run`/`github`, `tempRepo` for `git`, dependency-injection for modules with multiple I/O collaborators (`cleanup`, `terminal` spawn). CLI happy-path integration tests live in `tests/cli.integration.test.ts`; the bash and PowerShell runner scripts have their own integration suites; `scripts/install.py` is covered by pytest. Run the suite with `bun run test` — the `--max-concurrency=1` pin matters; see `tests/README.md` for fixture details and the rationale.
