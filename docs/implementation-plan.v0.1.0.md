...
One PR (initial seed → `dev`) lands all of v0.1.0. Commits below are the intended sequence inside that PR; the PR is opened only after the final commit. The repo follows `dev` → `prerelease/<name>` → `release/<name>`; there is no `main`.
...
- Initial commit lands on `dev`, which is the long-lived working branch and the base for every PR.
...
- `.github/workflows/ci.yml` — Ubuntu, `oven-sh/setup-bun`, runs `bun install`, `bun run lint`, `bun run typecheck`, `bun test`. Triggers on push to `dev` / `release/**` and PRs targeting those branches.
- `CHANGELOG.md` with v0.1.0 entry.
- Bump version in `package.json` to `0.1.0`.

## Phase 9 — Open the PR

- Push `dev` to `origin` (no `main` involved).
