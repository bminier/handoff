...
function writeFakeShims(opts: { fakeBinDir: string }): void {
  ...
  writeFileSync(
    ...
    [
      ...
      `// Any other gh invocation (or one missing the expected flags) exits`,
      `// non-zero so a regression in github.ts argv is caught loudly.`,
      ...
      `}`,
      `function bail(msg) {`,
      `  process.stderr.write('fake-gh: ' + msg + ': ' + argv.join(' ') + '\\n');`,
      `  process.exit(2);`,
      `}`,
      `if (argv[0] !== 'pr' || argv[1] !== 'list') {`,
      `  bail('unexpected subcommand');`,
      `}`,
      `// Validate the flags github.ts is supposed to send. If a regression`,
      `// drops --head, switches --state, or removes --json, the integration`,
      `// test would otherwise silently accept the malformed call.`,
      `if (argv.indexOf('--head') === -1) bail('missing --head');`,
      `const stateIdx = argv.indexOf('--state');`,
      `if (stateIdx === -1 || argv[stateIdx + 1] !== 'merged') {`,
      `  bail('missing or wrong --state');`,
      `}`,
      `if (argv.indexOf('--json') === -1) bail('missing --json');`,
